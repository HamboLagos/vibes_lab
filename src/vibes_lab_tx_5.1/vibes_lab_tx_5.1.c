...
*  &nbsp;&nbsp;&nbsp;&nbsp;<i>5.1</i>
 *
 *  The channel 3 Timer ISR has been significantly changed in order to improve
 *  the performance of the system overall. Rather than waiting for the ADC to
 *  complete a conversion inside the ISR, the ADC is set up to run at the SFR
 *  level, and an IRQ is additionally generated upon completion of an "extra"
 *  (ie. single) Conversion (only ADCCON3 allows an IRQ to be asserted upon
 *  conversion completion). The ADC ISR is then used to encode and save the
 *  conversion result in packet buffer memory.
 *
 *  <b>Results:</b><br>
 *  Code was implemented properly to give above performance improvement.
 */
...
ISR(ADC, 2)
{
   ...
   if(initComplete)
   {
      ...
      accSample = ADC >> 5;   // [3:0] of ADC are unreliable, we throw them away
                              ...
                              // of 1 more bit to get 10 bit result

      // Multiply accSample by 4. This is because, 10 bit ADC result is a number
      // between 0-1023, but dac wants 0-4095. Simplifies rx code if done here.
      // x <<= 2 is the same as x *= 2^2, but MUCH faster.
      accSample <<= 2;
      ...
      if(packet)
      {
         ...
         if(sampleIndex == RADIO_PAYLOAD_SIZE)
         {
            ...
            // get new packet to populate, reset sampleIndex and isrCounter
...
/*!\brief Sets status of LEDs on board Wixel.
 ...
 static void updateLeds()
 {
          ...
          && !lostSample)
    {
       ...
       operationalTimeStamp = getMs() + 3000;
       sendReceiveComplete = FALSE;
       lostSample = FALSE;
    }
 }
 ...
 {
    if(usbComTxAvailable() > 16)
    {
       static uint8 XDATA buffer[16];
       uint8 bufferLength;


       bufferLength = sprintf(buffer, "Trans Ready: %d\r\n", transmissionsReady);
